...
if __name__ == '__main__':
    # 使用你提供的路径作为日志默认路径
    train_log_path = os.environ.get('TRAIN_LOG_PATH', '/home/xjf/ad_algo/logs')
    # 其他环境变量保持默认或根据需要调整
    ...
    log_file = open(Path(train_log_path, 'train.log'), 'w')
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        train_ckpt_path = os.environ.get('TRAIN_CKPT_PATH', './checkpoints')  # 默认保存到当前目录的checkpoints文件夹
        save_dir = Path(train_ckpt_path, f"global_step{global_step}.valid_loss={valid_loss_sum:.4f}")
